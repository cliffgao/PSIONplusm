...
def api_pssm_features(pssmfn,fastaseq):
    ####
    con_features=[]
    if os.path.isfile(pssmfn):
        myarray,fastaseq=r_array(pssmfn)
    else:
        myarray,fastaseq=[],[]
        print("Not find PSSM for %s" %pssmfn)
        
    if len(myarray)==0 or len(fastaseq)==0:
        print( "### %s is not find " %pssmfn)
    #2d
    xcon_features=conservation_features(myarray,fastaseq)
    for i in range(len(xcon_features)):
        for j in range(len(xcon_features[i])):
            con_features.append(xcon_features[i][j])
    return(con_features)


def api_pssm_features_fr(fx_n,folder):

    # read in fasta file ; return protein id 
    fxlines=get_idx(fx_n)

    CONS=[] # each item is 2d list L*20
    for i in range(len(fxlines)):
        if len(fxlines[i])>0 :
            proid=fxlines[i]
            f_name="%s/%s.pssm" %(folder,proid)
            #f_name=folder+"/"+proid+".pssm" #2010-08-15
            if os.path.isfile(f_name):
                myarray,fastaseq=r_array(f_name)
            else:
                myarray,fastaseq=[],[]
                print("Not find PSSM for %s" %proid)
                
            if len(myarray)==0 or len(fastaseq)==0:
                print( "### %s is not find " %proid)
            con_features=[]
            con_features=conservation_features(myarray,fastaseq)
            CONS.append(con_features)
    # 3d list
    return(CONS)  
# =============================================================================
...
if __name__=="__main__":
    if len(sys.argv)<3:
        ...
        fx_n=sys.argv[1]
        folder=sys.argv[2]
        CONS=api_pssm_features(fx_n,folder)
        print(CONS)
