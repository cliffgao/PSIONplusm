...
class PSION_features:
    ### PSION is a model build by SVM
    def __init__(self,proseq,dir):
# =============================================================================
#         # read fasta file
#         fr=open(frn,'r')
...
# =============================================================================
        ...
        self.fcons=[]
        self.fdiaa=[]
        psipredfn="%s/target.ss" %dir
        disorderfn="%s/target.diso"%dir
        spinexfn="%s/spXout/target.spXout" %dir
        pssmfn="%s/target.pssm" %dir
# =============================================================================
#         ### for each fasta id
...
#                 proseq=lines[i+1]
        ...
        if os.path.isfile(spinexfn):
            self.spdAA,self.spdSS,self.spdBdEd,self.spdRSA=api_dowith_spinex(spinexfn,proseq)
        else:
            self.spdAA=""
            self.spdSS=""
            self.spdBdEd=""
            self.spdRSA=[]
        self.frsa=api_rsa_features(self.spdAA,self.spdSS,self.spdRSA)
        ### ### conservation scores
    ...
    def get_all_features(self):
        ### features used in PSIONplus
        all_features=[]
        for item in self.faacvpp49:
            all_features.append(round(item,5))
