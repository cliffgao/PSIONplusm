...
def merge_psion_blast(svm_label,qidx,label,svm_predict,MAP,counterP,counter):
	...
	#print(qidx+1,firstline,counter,votek,votev,MAP[votek],"ok")
	
def get_prob_from_vote(counter)	:
	vsum=0
	N=len(counter)-1 # last one is No hit 
	probs=[0.0 for k in range(N)]
	#probs in -1 1/ 1 2 3 4 order
	for k, v  in counter.items():
		if k!='N':
			vsum=vsum+v
	if vsum==0:#Not hit from blast 2015-05-22
		if N==2: #binary, not hit is -1.; non-ion, lig-gated
			probs=[1.0,0.0]
		else: #multi, no hit is 1 K
			probs=[1.0,0.0,0.0,0.0]
		return (probs)
	elif N==2: #Binary classifier
		#Binary="T"
		for k,v in counter.items():
			if k!='N': 
				if k=="lig" or k=="non_ion":# negative 
					probs[0]=v*1.0/vsum
				elif k=="vol" or k=="ion":#positive
					probs[1]=v*1.0/vsum
				else:
					print("not such k %s" %k)
	elif N==4:#multi		k':'1','anion':'2','ca':'3','na':'4	
		for k,v in counter.items():
			if k!='N':
				if k=="k":
					probs[0]=v*1.0/vsum
...
def prob2label(probs,Binary):
	...
	if idx>=0:
		...
		label="5"
	return label	
def prob2label2(counter,pcounter,Binary):
	#labels=["1","2","3","4" ]
...
#				probs=[0.0,1.0] # nohit is voltage-gated
	...
	if idx>=0:
		...
		label="5"
	return (label,probs)		
def get_label_prediction(fr_n,fw_n,MAP,psion_fn,Ecutoff):
	...
	for i in range(len(lines)):
		if len(lines[i])>0 and lines[i][0]=='>':
			...
			#write the blast prediction	
			#print(counter)
			#print(pcounter)				
			apredict,blastProb=prob2label2(counter,pcounter,Binary)
			#print(apredict,blastProb)
...
if __name__=="__main__":
	if len(sys.argv)<3:
		...
		if MAP is None:
			...
			get_label_prediction(fr_n,fw_n,MAP,psion_fn,Ecutoff)
