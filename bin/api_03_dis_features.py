...
def cv_ss(oneseq):
  ...
  return results
#-------------------
def api_dis_features(proseq,diseq):
  #predict disorder
  dis_features=[]
  #sss=diseq
  #ss segements CC HHH EEEE CC
  mysegments=ss_segment(diseq)

  # total number of ss fragments (1)
  dis_seg_num=total_ss(mysegments)
  dis_features.append(dis_seg_num)

  # number of segments in C/H/E  (2)
  ss_num_CHE=ss_nums(mysegments)
  for k in range(len(ss_num_CHE)):
    dis_features.append(ss_num_CHE[k])
  #min/max segment fraction of order/ Disordered  (2*2=4 )
  minmax_ss=percent_min_max_ss(mysegments,proseq)
  for k in range(len(minmax_ss)):
    dis_features.append(minmax_ss[k])
  
  # composition of ss (2)
  percent_ss=cv_ss(diseq)
  for k in range(len(percent_ss)):
    dis_features.append(percent_ss[k])
  return(dis_features)
